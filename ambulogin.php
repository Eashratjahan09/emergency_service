...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $AComp_ID = $_POST['AComp_ID'];
    $password = $_POST['password'];

$sql = "Select * from ambulance_company where AComp_ID='$AComp_ID' AND Password='$password'";
$connect = mysqli_connect( HOST, USER, PASS, DB );
$result = mysqli_query($connect, "Select * from ambulance_company where AComp_ID='$AComp_ID' AND Password='$password'");
$num = mysqli_num_rows($result);
if ($num == 1){
    session_start();
                    $_SESSION['loggedin'] = true;
                    $_SESSION['AComp_ID'] = $AComp_ID;
                    $_SESSION['Password'] = $password;
    header("location:Ambu/slnhome.php");

}
else{
    echo" Password or  AComp_ID is wrong";
}
  ...
  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
    ...
    <li><a class="dropdown-item" href="ambulogin.php">Ambulance</a></li>
        ...
        <form action="" method="POST" class="form" style="margin-left: 9vw">
            <label for="AComp_ID">Ambulance Co. ID: </label><br>
            <input style="width: 70%"type="number" name="AComp_ID"></input><br>
